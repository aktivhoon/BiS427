...
%% Simulate the EPSC (multiplicative rule)
EPSC_max=3; EPSC_min=0;
for jt=1:length(update_time)
    index=update_time(jt);
    tmp_dEPSC=STDP(10,10,delta_t(jt))/100; a_p=1/3; a_m=1/4;
    if tmp_dEPSC > 0
        d_EPSC(index)=tmp_dEPSC*(EPSC_max-EPSC(index-1))*a_p;
    end
    if tmp_dEPSC < 0
        d_EPSC(index)=tmp_dEPSC*(EPSC(index-1)-EPSC_min)*a_m;
    end
    if jt~=length(update_time)
        EPSC(index:update_time(jt+1)-1)=EPSC(index-1)+d_EPSC(index);
    end
    if jt==length(update_time)
        EPSC(index:end)=EPSC(index-1)+d_EPSC(index);
    end
end

EPSC_ratio=ones(length(d_EPSC),1);
...
xlim([0 T]); ylim([min(EPSC_ratio) max(EPSC_ratio)]);
